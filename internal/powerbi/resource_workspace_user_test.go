	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// first step creates the resource
			{
				...
				resource "powerbi_workspace_user" "test" {
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet("powerbi_workspace_user.test", "id"),
					resource.TestCheckResourceAttrSet("powerbi_workspace_user.test", "workspace_id"),
				),
			},
			// final step checks importing the current state we reached in the step above
			{
				ResourceName:      "powerbi_workspace_user.test",
				ImportState:       true,
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// first step creates the resource
			{
				Config: config,
				Check: resource.ComposeTestCheckFunc(
					set("powerbi_workspace_user.test", "id", &workspaceUserID),
			...
			// second step skew new access right
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testCheckGroupUserExistsInWorkspace("powerbi_workspace.test", "vijaykumar.u@nuance.com"),
					resource.TestCheckResourceAttr("powerbi_workspace_user.test", "group_user_access_right", "Admin"),
				),
			},
